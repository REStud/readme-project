function  [labels,data] = readColData(fname,ncols, nrows, nhead,nlrows)

% used to read text files with labels.
% from http://www.me.pdx.edu/~gerry/MATLAB/plotting/loadingPlotData.html
...
% 
% 
% data = reshape(data,ncols,nrows)';   %  notice the transpose operator
% x = data(:,1);
% y = data(:,2:ncols);
% 
...
%--------------------------------------------------------
% modifed by myself since data is not a rectangle
data=zeros(ncols, 1);
x = fscanf(fin,'%f',ncols);
data=x;

for i=2: nrows
   x = fscanf(fin,'%f',ncols);  %  Load the numerical values into one long vector
   data=[data x];
end
